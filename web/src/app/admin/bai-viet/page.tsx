...
interface Post { id: string; slug: string; title: string; category: string; status: string; read_time: string; published_at: string }
            ...
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-bg/50 transition-colors">
                <td className="px-4 py-3">
                  ...
                  <div className="text-text-muted text-[11px]">{p.read_time} phút đọc</div>
                </td>
                <td className="px-4 py-3 hidden md:table-cell">
                  <span className="text-[10px] tracking-widest text-gold border border-gold/40 px-2 py-0.5">{p.category}</span>
                </td>
                <td className="px-4 py-3 text-text-muted text-[12px] hidden md:table-cell">{formatDate(p.published_at)}</td>
                <td className="px-4 py-3">
