...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/projects?status=all').then(r => r.json()),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-surface border border-border rounded-sm p-6">
          ...
          <ul className="space-y-2 text-sm text-text-muted">
            ...
            <li className="flex items-start gap-2"><span className="text-gold mt-0.5">•</span>Dữ liệu được lưu trực tiếp vào file JSON trên server</li>
          </ul>
        </div>
      </div>
    </div>
