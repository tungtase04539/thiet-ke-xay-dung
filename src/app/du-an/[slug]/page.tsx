    ...
    <>
      ...
      <main>
        ...
        <section className="pb-16 max-w-7xl mx-auto px-6">
          <div className="inline-flex items-center gap-3 mb-8">
            <span className="w-8 h-px bg-gold" />
            <span className="text-[11px] tracking-[0.3em] text-gold uppercase">Trước & Sau Thi Công</span>
          </div>
          <div
            className="relative rounded-sm overflow-hidden cursor-ew-resize select-none"
          ...
          >
            {/* Before (empty/raw) */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1200&q=80" alt="Trước"
              className="absolute inset-0 w-full h-full object-cover" />
            {/* After (finished) */}
            <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPos}%` }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={images[0]} alt="Sau" className="absolute inset-0 object-cover" style={{ width: `${10000/sliderPos}%`, maxWidth: 'none' }} />
            </div>
            {/* Label */}
            <div className="absolute top-4 left-4 text-[10px] tracking-widest bg-bg/70 text-text-muted px-2 py-1">TRƯỚC</div>
            <div className="absolute top-4 right-4 text-[10px] tracking-widest bg-bg/70 text-gold px-2 py-1">SAU</div>
            {/* Slider handle */}
            <div className="absolute top-0 bottom-0 w-0.5 bg-gold" style={{ left: `${sliderPos}%` }}>
              <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-gold flex items-center justify-center shadow-lg">
                <ChevronLeft size={10} color="#0A0A0F" />
                <ChevronRight size={10} color="#0A0A0F" />
              </div>
            </div>
          </div>
        </section>
